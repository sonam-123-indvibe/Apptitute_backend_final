...
exports.submitTest = async (req, res) => {
  try {
    ...
    for (const qId in answers) {
      // Skip invalid or static IDs
      if (!qId || qId === "undefined" || qId.startsWith("static_")) {
        console.log("Skipping invalid qId:", qId);
        continue;
      }

      const mongoose = require("mongoose");
      if (!mongoose.Types.ObjectId.isValid(qId)) {
        console.log("Skipping non-ObjectId qId:", qId);
        continue;
      }
    ...
    // Send email for all MCQ tests
    const user = await User.findById(userId);
    console.log("User found:", user);
    console.log("User email:", user?.email);
    if (user && user.email) {
      const total = questionsList.length;
      const correct = mcqScore;
      const wrong = formatted.filter(a => a.isCorrect === false).length;
      const skipped = total - formatted.length;

      await sendEmail(
        user.email,
        `${mcqScore} / ${total}`,
        `Your ${stream} ${level} Level result is here!`,
        { userName: user.name, correct, wrong, skipped, total }
      );
    }
